...
export default function ShopPage() {
  ...
  const categoryList = ['All', 'Home Decor', 'Bags & Totes', 'Dining & Runners', 'Bamboo Crafts', 'Cane Products', 'Gift Items'];

  return (
    <div className="container mx-auto px-4 py-8">
      {/* Breadcrumbs */}
      <div className="text-sm text-muted-foreground mb-6">
        <Link href="/" className="hover:text-primary">Home</Link> &gt; <span className="text-foreground">Shop</span>
      </div>
          ...
          <div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between text-sm text-muted-foreground">
                <span>₹0</span>
                <span>₹{maxPrice}{maxPrice === 10000 ? '+' : ''}</span>
              </div>
            </div>
          </div>
        ...
        <div className="flex-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredProducts.length > 0 ? filteredProducts.map(product => (
              <div key={product.id} className="group flex flex-col bg-card rounded-lg overflow-hidden border border-border shadow-sm hover:shadow-md transition-all">
                {/* Image Area */}
